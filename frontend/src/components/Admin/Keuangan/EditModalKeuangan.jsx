...
const EditModalKeuangan = ({ closeModal, onUpdate, transactionId }) => {
      ...
      const getTransactionById = async () => {
        try {
          const response = await axios.get(
            `http://localhost:3500/transaksi/${transactionId}`
          );
  ...
  try {
        await axios.put(`http://localhost:3500/transaksi/${transactionId}`, {
      type,
      category,
      amount,
      date: new Date(),
      ...
      <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 overflow-y-auto max-h-screen">
        <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            Form Ubah {type === "pengeluaran" ? "Pengeluaran" : "Pemasukan"}
          </h3>
          <button
            type="button"
        ...
        <form className="p-4 md:p-5" onSubmit={editPemasukkan}>
          ...
          >
            Ubah {type === "pengeluaran" ? "Pengeluaran" : "Pemasukan"}
          </button>
        </form>
      </div>
    </div>
...
export default EditModalKeuangan;

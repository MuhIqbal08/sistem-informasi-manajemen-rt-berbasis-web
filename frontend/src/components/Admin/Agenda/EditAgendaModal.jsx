// Masih Belum Sempurna (karena harus semua body terisi)
import axios from "axios";
import { Calendar, Clock, Info, MapPin, Type } from "lucide-react";
import React, { useEffect, useState } from "react";
import { useSelector } from "react-redux";
import { toast } from "react-toastify";

const EditAgendaModal = ({ closeModal, uuid, onUpdate }) => {
    ...
    const getAgendaById = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3500/agenda/${uuid}`
        );
  ...
  const editAgenda = async (e) => {
    e.preventDefault();
    try {
      const fullDateTime = new Date(`${tanggal}T${waktu}`);
      const formattedDateTime = fullDateTime
        .toISOString()
        .slice(0, 26)
        .replace("T", " ");

      await axios.put(`http://localhost:3500/agenda/${uuid}`, {
        userId: user.uuid,
        title: judul,
        deskripsi,
  ...
  return (
    <div className="fixed z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md">
      <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 overflow-y-auto max-h-screen">
        ...
        <form className="p-4 md:p-5" onSubmit={editAgenda}>
          ...
          <div className="grid gap-4 mb-4">
            ...
            <div className="col-span-2">
              <label
                htmlFor="time"
                className="flex items-center text-sm font-medium text-gray-700 gap-2 mb-2"
              >
                <Clock size={16} />
                Time
              </label>
          ...
          <button
            type="submit"
            className="text-white w-full bg-blue-600 hover:bg-indigo-600 focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800 transition-all duration-200"
          >
            Ubah Agenda
          </button>
        </form>

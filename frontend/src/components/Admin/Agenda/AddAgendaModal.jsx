...
import { Calendar, Clock, Info, Locate, MapPin, Type } from "lucide-react";
import React, { useState } from "react";
import { useSelector } from "react-redux";
import { toast } from "react-toastify";

const AddAgendaModal = ({ closeModal, onUpdate }) => {
  ...
  const addAgenda = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:3500/agenda", {
        userId: user.uuid,
        title: judul,
        deskripsi,
        tanggal,
  ...
  return (
    <div className="fixed z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md">
      <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 overflow-y-auto max-h-screen">
        ...
        <form className="p-4 md:p-5" onSubmit={addAgenda}>
          ...
          <button
            type="submit"
            className="text-white w-full bg-blue-600 hover:bg-indigo-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800 transition-all duration-200"
          >
            Tambah Agenda
          </button>
        </form>

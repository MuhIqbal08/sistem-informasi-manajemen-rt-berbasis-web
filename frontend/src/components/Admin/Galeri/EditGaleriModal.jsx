...
const EditGaleriModal = ({ closeModal, uuid, onUpdate }) => {
    ...
    const getGaleriById = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3500/galeri/${uuid}`
        );
        const galeri = response.data;
        setJudul(galeri.judul);
        setImage(galeri.image);
        // setWaktu(galeri.waktu);
        console.log("dataId: ", galeri);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const editGaleri = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:3500/galeri/${uuid}`, {
        userId: user.uuid,
        judul: judul,
        image: image || null,
      });
      toast.success("Galeri Berhasil DiUbah!");
      onUpdate();
      closeModal(); 
    } catch (error) {
  ...
  return (
    <div className="fixed z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md">
      <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 overflow-y-auto max-h-screen">
        ...
        <form className="p-4 md:p-5" onSubmit={editGaleri}>
          ...
          <button
            type="submit"
            className="text-white w-full bg-blue-600 hover:bg-indigo-600 font-medium transition-all duration-200 rounded-lg text-sm px-5 py-2.5 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
          >
            Ubah Galeri
          </button>
        </form>
